(function($) { "use strict";
	...
	if (jQuery(".search-click").length) {
		jQuery(".search-click").on('touchstart click', function(){
			...
			jQuery(".discy-content,.main-content").on("click",function () {
	...
	panel_pop("#lost-password",".lost-password,.discy_for_all .reset-password,.main_for_all .reset-password");
	panel_pop("#lost-password",".lost-passwords","no",".discy_for_all,.main_for_all");
	...
	jQuery(document).on("click",".poll-submit",function() {
		...
		if (poll_val.length == 0) {
			...
			var poll_title     = (wpqa_custom.poll_image_title == 'on'?poll_val.attr("data-rel").replace("poll_",""):"");
